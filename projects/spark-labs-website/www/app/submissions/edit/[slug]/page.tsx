...
function Page({ params }: { params: { slug: string } }) {
    ...
    function regexClassroomId(input: string, regex: RegExp): any {
    ...
    const submitProject = async () => {
        ...
        const matchingUser = users.find((user: any) => auth.currentUser && auth.currentUser.uid === user.userId);
        const userIdentificationLatest = matchingUser ? matchingUser.id : "";

        const Create = await addDoc(collection(db, "submissions"), {
            xml: ide.getProjectXML(),
            title: title,
            description: description,
            thumbnail: thumbnail,
            classroomId: regexClassroomId(params.slug, regex),
            userId: userIdentificationLatest,
        ...
        setSubmitBar(false);
        toast({
            title: "Submission has been added!",
            description: (
    ...
    useEffect(() => {
        if (worldRef.current) {
            const world: any = new WorldMorph(worldRef.current) as any;
            const ideInstance: any = new IDE_Morph({
                hideProjects: true,
                noCloud: true,
                noExitWarning: true,
                design: "flat" // classic | flat
            }) as any;
    ...
    return (
        <>
            ...
            <Link href="/dashboard" className="flex-center fixed top-0 left-0 w-[152.25px] h-[28.25px] z-50 bg-background border">
            ...
            <div className={cn("min-h-max w-[345px] rounded-md fixed bottom-12 left-[calc(50%-172.5px)] transform items-end bg-background hidden", navigationsBar ? "scale-in-ver-bottom" : "slide-out-blurred-top")}>
                <div className="h-full w-full p-3 border rounded-md flex flex-col space-y-3 pb-12 !font-mono !tracking-tighter">
                    ...
                    }} href='/dashboard' className="flex items-center w-full justify-between p-3 border rounded-md hover:bg-primary hover:text-primary-foreground">
                        <span>Dashboard</span>
