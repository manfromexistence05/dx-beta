                                    ...
                                    <div className="flex-1 flex items-end justify-end gap-3">
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent className="w-56">
                                                {docs.map((classroom: any) => classroom.students.some((student: any) => student === user.id) ? <Link href={`submissions/edit/${classroom.id}+${user.userId}`} key={classroom.id}><DropdownMenuItem>{classroom.title || "No title"}</DropdownMenuItem></Link> : <DropdownMenuItem key={classroom.id}>You are not joined in any classrooms yet.</DropdownMenuItem>)}

                                                {/* {
                                                    docs.map((classroom: any) =>
                                                        classroom.students.some((student: any) => student === user.id)
                                                    ) ? null : <DropdownMenuItem>You +are not joined in any classrooms yet.</DropdownMenuItem>
                                                } */}
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                    </div>
                                                ...
                                                docs.map((classroom: any) => {
                                                    ...
                                                    return (
                                                        <div key={classroom.id} className={cn("w-full flex flex-row justify-between items-center text-sm font-mono", isStudentInClassroom ? "hover:bg-primary-foreground hover:text-primary gap-3  p-3" : "")}>
                                                            ...
                                                            ) : <div className="flex-center w-full min-h-[200px] hover:bg-primary hover:text-primary-foreground">No result.</div>
                                                            }
                                                        </div>
                                                    );
                                                })
                                            ...
                                            <CardContent className="px-6 space-y-4 min-h-[200px] py-5 overflow-x-hidden overflow-y-auto">
                                                ...
                                                <div className="flex flex-col flex-1 h-auto gap-3">
                                                    ...
                                                    <Link href={`submissions/presentation/${items.id}`}>
                                                        <Button className="w-full bg-[#E4ACB2] hover:bg-[#E4ACB2] text-[#000000] hover:text-white hover:text-[#ffffff]">
                                                            Run This Project
                                                        </Button>
                                                    </Link>
